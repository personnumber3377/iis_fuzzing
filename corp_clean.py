...
# This file basically tries to just filter out the good testcases from the bad ones by doing the shit...
...
def save_corp(corpus_list, save_dir):
    assert "/" not in save_dir
    for i, testcase in enumerate(corpus_list):
        fh = open("./"+str(save_dir)+"/"+str(i), "wb")
        fh.write(testcase)
        fh.close()
    print("[+] Saved good corpus!!!!")
    return
